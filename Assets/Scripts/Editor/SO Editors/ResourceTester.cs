...
class ResourceTester : IPreprocessBuildWithReport
{
    ...
    {
        bool succes = true;
        BuildingData data = AssetDatabase.LoadAssetAtPath<BuildingData>($"Assets/Game Data/Research and Building/{BuildingData.PATH}.asset");
        for (int i = 0; i < data.Categories.Count; i++)
        {
            for (int j = 0; j < data.Categories[i].Objects.Count; j++)
            {
                ...
                if (building != null)
                {
                    ...
                    if (res != null)
                    {
                        if (CheckResource(res.ResourceYield, building.Name, data.Categories[i].Name, "Production cost"))
                            succes = false;
                        if (CheckResource(res.ResourceYield, building.Name, data.Categories[i].Name, "Production Yeild"))
                            succes = false;
                    }
                }
            }
    ...
    {
        bool succes = true;
        ResearchData data = AssetDatabase.LoadAssetAtPath<ResearchData>($"Assets/Game Data/Research and Building/{ResearchData.PATH}.asset");
        for (int i = 0; i < data.Categories.Count; i++)
        {
            for (int j = 0; j < data.Categories[i].Objects.Count; j++)
            {
